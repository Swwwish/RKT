    ...
    for i in range(batch):
        ...
        for j in range(seqlen):
            for k in range(seqlen):
                #print(qseqs_cpu[i][j], corr_dict[qseqs_cpu[i][j]])
...
def attention(query, key, value, rel, lambda_, timestamp, mask, dropout, su):
    ...
    if mask is not None:
        ...
        time_stamp= torch.exp( - torch.abs(timestamp.float()) / su )
        time_stamp=time_stamp.masked_fill(mask, -1e5)


    prob_attn = F.softmax(scores, dim=-1)
    r_attn = F.softmax(time_stamp + rel_attn, dim=-1)
    
    prob_attn = lambda_ * prob_attn + (1-lambda_) * r_attn
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, total_size, num_heads, drop_prob):
        ...
        self.su = nn.Parameter(torch.rand(1))

    def forward(self, query, key, value, rel, lambda_, timestamp, encode_pos, mask=None):
        ...
        '''
        out, self.prob_attn = attention(query, key, value, rel, lambda_, timestamp, mask, self.dropout, self.su)
        # concatenate all heads and put through final linear layer
        out = out.transpose(1, 2).contiguous().view(batch_size, seq_length, self.total_size)
        
        return out, self.prob_attn


class RKT_paper(nn.Module):
    def __init__(self, num_c, num_items, embed_size, num_attn_layers, num_heads, batch_size, dataset_name,
                  encode_pos, grad_clip, theta, lambda_, seqlen=200, drop_prob=0.1, time_span=100000, emb_type="qid", emb_path=""):
        ...
        """
        super(RKT_paper, self).__init__()
        self.model_name = "rkt_paper"
        ...
        self.pos_embeds = nn.Embedding(seqlen, embed_size*2 )
    ...
    def forward(self, dcur, rel_dict, train=True):
        ...
        if self.dataset_name in ["statics2011", "poj"]:
            ...
            indexs = torch.arange(q_data.size(-1)).cuda()
            ...
            indexs = torch.arange(pid_data.size(-1)).cuda()
        ...
        inputs = inputs + self.pos_embeds(indexs).repeat(inputs.shape[0],1,1)
        inputs = F.relu(self.lin_in(inputs))
        
        mask = future_mask(inputs.size(-2))
        
        time = computeTime(timestamp, self.time_span)
        ...
        outputs, attn  = self.attn_layers[0](query, inputs, inputs, rel, self.lambda_, time, 
                                             self.encode_pos, mask)
        outputs2 = query + self.dropout1((outputs)) # 残差1
        outputs2 = self.layer_norm1(outputs2) # layer norm1
        
        outputs_linear = self.linear2( self.dropout1(     # FFN
            self.activation(self.linear1(outputs2))) )
        outputs2 = outputs2 + self.dropout2((outputs_linear)) # 残差2
        outputs2 = self.layer_norm2(outputs2) # lay norm2
        
        for l in self.attn_layers[1:]:
            residual, attn = l(query, outputs2, outputs2, rel, self.lambda_, time, 
                               self.encode_pos, mask)
            #outputs = self.dropout2(outputs + F.relu(residual))
            outputs2 = outputs2 + self.dropout1((residual)) # 残差1
            outputs2 = self.layer_norm1(outputs2) # layer norm1
            
            outputs_linear = self.linear2( self.dropout1( # FFN
                self.activation(self.linear1(outputs2))) )
            
            outputs2 = query + self.dropout2((outputs_linear)) # 残差2
            outputs2 = self.layer_norm2(outputs2) # lay norm2
